...
% ---------------------------------------------------------- 1 ----------------------------------------------------------
ode = diff(y, x) == (x+y+1)/(2*x); % Definição da função
x0 = 2; y0 = 4; % Definição do PVI
cond = y(x0) == y0; % Definição do ponto para o symbolic
func = @(x,y) (x+y+1)/(2*x); % Definição da função para o Octave
leggraf = 'EDO: dy/dx = (x+y+1)/(2*x) - Valor inicial: y(2) = 4 -- Sol: y(x) = 3*sqrt(x*2)/2 + x - 1'; % String aleatoria do título do gráf.
...
% fprintf("-------------%d--------------", 4);
% ---------------------------------------------------------- 5 ----------------------------------------------------------
legend(leg,'Location', 'southeast'); % Add legenda
